<template>
  <div class="mine">
      <header data-v-172bb70c="" class="hd">
            <div data-v-172bb70c="" class="img">
                <img v-if="username" data-v-172bb70c="" src="../../assets/tx.jpg">
                <img v-else data-v-172bb70c="" src="https://m.mi.com/static/img/avatar.76a75b8f17.png">
            </div>
            <div v-if="username" data-v-172bb70c="" class="name">
                {{username}}
            </div>
             <div v-else @click="login" data-v-172bb70c="" class="name">
                登录/注册
            </div>
       </header>
       <div data-v-172bb70c="" class="b1">
           <div class="cite" @click="out">退出登录</div>
           <div class="span ui-flex align-center">
               <!-- <a href="http://localhost:8080/car">
               全部订单&nbsp;&nbsp;>
               </a> -->
               </div>
        </div>
        <div class="param">
            <div>
                <img src="data:image/png;base64," alt="">
                <span>待付款</span>
            </div>
            <div>
                <img src="data:image/png;base64," alt="">
                <span>待收货</span>
            </div>
            <div>
                <img src="https://m.mi.com/static/img/nav-4.d68723895f.png" alt="">
                <span>退换修</span>
            </div>
        </div>
        <div style="background:#f5f5f5;height:0.08rem;width:100%"></div>
        <div class="a">
            <a href="">
                <div class="left"><img src="data:image/png;base64," alt=""></div>
                <div class="right" style="borderBottom:1px solid #ccc;" >
                    <span>会员中心</span>
                    <span>></span>
                </div>
            </a>
            <a href="">
                <div class="left"><img src="data:image/png;base64," alt=""></div>
                <div class="right" >
                    <span>我的优惠</span>
                    <span>></span>
                </div>
            </a>
            <div style="background:#f5f5f5;height:0.08rem;width:100%"></div>
            <a href="">
                <div class="left"><img src="data:image/png;base64," alt=""></div>
                <div class="right" style="borderBottom:1px solid #ccc;" >
                    <span>服务中心</span>
                    <span>></span>
                </div>
            </a>
            <a href="">
                <div class="left"><img src="data:image/png;base64," alt=""></div>
                <div class="right" >
                    <span>小米之家</span>
                    <span>></span>
                </div>
            </a>
            <div style="background:#f5f5f5;height:0.08rem;width:100%"></div>
            <a href="">
                <div class="left"><img src="data:image/png;base64," alt=""></div>
                <div class="right" >
                    <span>F码通道</span>
                    <span>></span>
                </div>
            </a>
            <div style="background:#f5f5f5;height:0.08rem;width:100%"></div>
            <a href="">
                <div class="left"><img src="https://m.mi.com/static/img/i-setting.fb9625b3f2.png" alt=""></div>
                <div class="right" >
                    <span>设置</span>
                    <span>></span>
                </div>
            </a>
            <div style="background:#f5f5f5;height:0.5rem;width:100%;flex:1"></div>
        </div>
      <tabbar></tabbar>
  </div>
</template>

<script>
// @ is an alias to /src

import tabbar from '../../components/tabbar/tabbar'
export default {
    name:'mine',
    components:{
        tabbar
    },
    data() {
        return {
            username:''
        }
    },
    methods: {
        login(){
            this.$router.push("/login")
        },
        out(){
            sessionStorage.username = "";
            this.username = "";
        }
    },
    created() {
        this.username = sessionStorage.username;
    },
}
</script>
<style lang="scss" scoped>
    .mine{
        background:#f5f5f5;
        height: 100%;
        display: flex;
        flex-direction: column;
    }
    .hd{
        background: url(https://m.mi.com/static/img/bg.63c8e19851.png) center 0 #f37d0f;
        background-size: auto 100%;
        padding: .26rem .15rem;
        display: flex;
        align-items: center;
        .img{
            width: .44rem;
            height: .44rem;
            margin-right: .1rem;
            img{
                width: 100%;
                height:100%;
                border-radius: 50%;
            }
        }
        .name{
            font-size: .16rem;
            color: white;
        }
    }
    .b1{
        background-color: #fff;
        height: .4rem;
        border-bottom: 1px solid rgba(0,0,0,.15);
        color: rgba(0,0,0,.54);
        position: relative;
        padding: 0 .15rem;
        display: flex;
        justify-content: space-between;
        align-items: center;
        color: black;
        a{
            color: black;
        }
    }
    .param{
        height: .9rem;
        background: white;
        display: flex;
        justify-content: space-between;
        align-items: center;
        padding:0 0.4rem;
        border-top: 1px #ccc solid;
        border-bottom: 1px #ccc solid;
        div{
            display: flex;
            flex-direction: column;
            align-items: center;
            height:auto;
            img{
                width:.25rem;
                height:.24rem;
                margin-bottom: .08rem;
            }
            span{
                font-size: .12rem;
            }
        }
    }
    .a{
        width:100%;
        a{
            display: flex;
            height:.54rem;
            width: 100%;
            justify-content: space-between;
            align-items: center;
            padding:0 .15rem;
            background: white;
            .left{
                width:.3rem;
                height:.3rem;
                display: flex;
                justify-content: flex-start;
                img{
                    width: 100%;
                    height:100%;
                }
            }
            .right{
                margin-left:.15rem;
                flex: 1;
                height:100%;
                line-height: .54rem;
                display: flex;
                justify-content: space-between;
                color: black;
                .inline{
                    border-bottom:1px solid #ccc;
                }
                &:nth-of-type(3){
                    border-bottom:1px solid #ccc;
                }
            }
        }
    }
</style>

